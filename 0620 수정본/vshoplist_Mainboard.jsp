...
<%
	...
	ArrayList<ProductDTO> dtos = productDAO.vshoplistSetting(3);
	...
	<div class="row">
		...
		<span class = "btnspan" style = "width : 50px">
			<input id="<%=dto.getName()%>_<%=dto.getPrice()%>_<%=dto.getProductId()%>" type = "button" value = "선택" class = "btn btn-danger" onclick="tdSet(this.id)">
			<script type="text/javascript">
				//선택 누를 시 td안의 값 변경
				function tdSet(id) {
					var arrayId = id.split("_")
					$(parent.document).find('#tdMainboardName').html(arrayId[0]);
					$(parent.document).find('#tdMainboardPrice').html(arrayId[1]);
					$(parent.document).find('.mainboardId').val(arrayId[2]);
				}
			</script>
		</span>
	</div>
